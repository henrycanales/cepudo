<?php
  ...
  $items = $_POST['items'];
    ...
    if($dato == "Todo" && $dato2 != "Todo"){
            ...
            $nroProductos = mysqli_num_rows(mysqli_query($conexion,"SELECT * FROM AjusteInventario where Autorizacion = '$dato2' "));

    }
    if($dato != "Todo" && $dato2 != "Todo"){

    $nroProductos = mysqli_num_rows(mysqli_query($conexion,"SELECT * FROM AjusteInventario where TipoAjuste = '$dato' and Autorizacion = '$dato2' "));
    ...
    if($dato == "Todo" && $dato2 == "Todo"){  
     ...
     if($dato == "Todo" && $dato2 != "Todo"){
        $registro = mysqli_query($conexion,"SELECT * FROM AjusteInventario where Autorizacion = '$dato2' Order By AjusteId Desc LIMIT $limit, $nroLotes ");
      }

     if($dato != "Todo" && $dato2 != "Todo"){
        $registro = mysqli_query($conexion,"SELECT * FROM AjusteInventario where TipoAjuste = '$dato' and Autorizacion = '$dato2' 
  ...
  while($registro2 = mysqli_fetch_array($registro)){
    ...
    $tabla = $tabla.'<tr>
              ...
              <td>
              <a <button onclick="autorizarAjuste('.$registro2['AjusteId'].');"class="btn glyphicon glyphicon-ok" alt="Autorizar Ajuste" title="Autorizar Ajuste"></button></a>
