<?php
  ...
  $items = $_POST['items'];
    ...
    $nroProductos = mysqli_num_rows(mysqli_query(
            $conexion,"SELECT * FROM usuarios where  Nombre LIKE '%dato%'
            or UsuarioId LIKE '%$dato%'"));
    }else{
...
$registro = mysqli_query(
            $conexion,"SELECT * FROM usuarios where UsuarioId LIKE '%$dato%' 
            or Nombre LIKE '%$dato%' LIMIT $limit, $nroLotes ");
