$('document').ready(function(){ 
	 ...
	 $("#formulario").validate({
      rules:
	  {
       ...
       messages:
	   {
            ...
            descripcionDonacion: "Ingrese Descripcion de la Donacion.!",
            fechaDonacion: "Ingrese Fecha.!",
            poDonacion: "Ingrese PO.!",
            cantidadContenedores: "Ingrese Cantidad de Contenedores.!",
            tamanoContenedores:"Ingrese Tamano de Contenedores.!",
	...
	$('#nuevo-producto').on('click',function(){
		...
		var campo = document.getElementById('donante');
		campo.style.display = "";
...
function agregaRegistro(){
	...
	var pag=1;
	var url = '../PHP/AgregarDonacion.php';
	$.ajax({
		type:'POST',
		url:url,
...
function editarRegistro(id){
	...
	var url = '../PHP/EditarDonacion.php';
		$.ajax({
		type:'POST',
		url:url,
		data:'id='+id,
		success: function(valores){
				...
				$('#pro').val('Edicion');
				$('#id-prod').val(id);
				$('#descripcionDonacion').val(datos[0]);
				$('#fechaDonacion').val(datos[1]);
				$('#poDonacion').val(datos[2]);
				$('#cantidadContenedores').val(datos[3]);
				$('#tamanoContenedores').val(datos[4]);
				$('#registra-producto').modal({
					show:true,
					backdrop:'static'
				});
...
function eliminarRegistro(id){
	var pag=1;
	var url = '../PHP/EliminarDonacion.php';
	var items = document.getElementById("Items").value;
